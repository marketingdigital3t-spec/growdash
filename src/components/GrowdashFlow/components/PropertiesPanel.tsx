...
export function PropertiesPanel({ element, selectionCount, onChange, onDuplicate, onDelete, onFront, onBack, onClose }: {
  ...
  return <aside aria-label="Propriedades do elemento" className="absolute bottom-16 left-3 top-28 z-30 w-[min(280px,calc(100%-1.5rem))] overflow-y-auto rounded-2xl border border-black/10 bg-white/96 p-4 text-slate-900 shadow-[0_22px_60px_-25px_rgba(0,0,0,.35)] backdrop-blur-xl dark:border-primary/20 dark:bg-[#090909]/96 dark:text-white 2xl:top-16">
    <header className="mb-4 flex items-start justify-between gap-3"><div><span className="text-[9px] font-black uppercase tracking-[.18em] text-primary">Propriedades</span><h3 className="mt-1 truncate text-sm font-black">{selectionCount > 1 ? `${selectionCount} elementos` : element.type}</h3></div><Button variant="ghost" size="icon" className="h-7 w-7 text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-white/55 dark:hover:bg-white/5 dark:hover:text-white" onClick={onClose}><X className="h-4 w-4" /></Button></header>
