import { ArrowDownToLine, ArrowUpToLine, Copy, Lock, Maximize2, Trash2, Unlock, X } from "lucide-react";
...
export function PropertiesPanel({ element, selectionCount, onChange, onDuplicate, onDelete, onFront, onBack, onClose }: {
  ...
  const numberChange = (key: keyof DrawElement) => (event: React.ChangeEvent<HTMLInputElement>) => onChange({ [key]: Number(event.target.value) || 0, ...(key === "width" && element.type === "text" ? { autoSize: false } : {}) });
  return <aside aria-label="Propriedades do elemento" className="growdash-flow-chrome growdash-flow-properties absolute bottom-16 left-3 top-28 z-30 w-[min(280px,calc(100%-1.5rem))] overflow-y-auto rounded-2xl p-4 2xl:top-16">
    ...
    {(element.type === "text" || element.type === "sticky") && <><p className="mt-4 text-xs leading-relaxed text-muted-foreground">Textos acompanham o conteúdo. Ao ajustar a largura pelas alças laterais, você passa a controlar as quebras de linha.</p>{element.type === "text" && <Button variant="outline" size="sm" className="growdash-flow-secondary-action mt-3 w-full" onClick={() => onChange({ autoSize: true })}><Maximize2 className="mr-1.5 h-3.5 w-3.5" />Ajustar ao texto</Button>}<PropertySlider label={`Texto · ${element.fontSize || 20}px`} value={element.fontSize || 20} min={10} max={72} onChange={(value) => onChange({ fontSize: value })} /></>}
