...
export function TopBar({
  ...
  const controlClass = "h-9 w-9 shrink-0 text-slate-600 hover:bg-primary/10 hover:text-primary dark:text-white/70 dark:hover:bg-primary/10 dark:hover:text-primary";
  return <>
    <div aria-label="Ações do quadro" className="absolute right-3 top-3 z-30 flex items-center gap-1 rounded-2xl border border-black/10 bg-white/95 p-1.5 shadow-[0_12px_35px_-18px_rgba(0,0,0,.35)] backdrop-blur-xl dark:border-primary/20 dark:bg-[#090909]/94">
      <Button variant="ghost" size="icon" className={cn(controlClass, showGrid && "bg-primary/10 text-primary")} onClick={onToggleGrid} title="Mostrar ou esconder grade"><Grid3X3 className="h-4 w-4" /></Button>
      <button type="button" onClick={onToggleSnap} className={cn("h-9 shrink-0 rounded-xl px-2 text-[10px] font-black uppercase tracking-wide text-slate-500 hover:bg-primary/10 hover:text-primary dark:text-white/55", snapToGrid && "bg-primary/10 text-primary")} title="Encaixar na grade">Snap</button>
      <AlertDialog>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild><Button variant="ghost" size="sm" className="h-9 shrink-0 gap-1.5 text-slate-600 hover:bg-primary/10 hover:text-primary dark:text-white/70"><Download className="h-4 w-4" /><span className="hidden 2xl:inline">Exportar</span></Button></DropdownMenuTrigger>
    ...
    <div aria-label="Zoom e histórico" className="absolute bottom-3 left-3 z-30 flex items-center gap-1 rounded-2xl border border-black/10 bg-white/95 p-1.5 shadow-[0_12px_35px_-18px_rgba(0,0,0,.35)] backdrop-blur-xl dark:border-primary/20 dark:bg-[#090909]/94">
