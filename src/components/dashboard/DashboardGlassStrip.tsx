...
export function DashboardGlassStrip({ revenue, spend, leads, cpl, roas, forecast30, openAlerts, sales }: Props) {
  ...
  return <section className="sticky top-[calc(88px+env(safe-area-inset-top))] z-20 min-w-0 lg:top-[49px]" aria-label="Resumo fixo do Dashboard">
    <div className="overflow-hidden rounded-2xl border border-white/20 bg-background/72 shadow-[0_10px_36px_rgba(0,0,0,.18)] backdrop-blur-2xl supports-[backdrop-filter]:bg-background/58 dark:border-[#8c6b32]/25 dark:bg-[#100c07]/82 dark:shadow-[0_10px_38px_rgba(0,0,0,.42),inset_0_1px_0_rgba(255,226,138,.06)]">
      ...
      {!compact && <div className="grid min-w-0 grid-cols-2 gap-px border-t border-border/50 bg-border/45 md:grid-cols-4 md:border-t-0 xl:grid-cols-8">{metrics.map((metric) => <article key={metric.label} className="flex min-w-0 items-center gap-2 bg-background/75 px-3 py-3 dark:bg-[#0d0905]/82"><span className={cn("grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-primary/10 text-primary [&_svg]:h-4 [&_svg]:w-4", metric.tone === "good" && "bg-emerald-500/10 text-emerald-500", metric.tone === "warn" && "bg-amber-500/10 text-amber-500")}>{metric.icon}</span><div className="min-w-0"><p className="truncate text-[8px] font-black uppercase tracking-[.08em] text-muted-foreground" title={metric.label}>{metric.label}</p><p className="truncate text-xs font-black tabular-nums sm:text-sm" title={metric.value}>{metric.value}</p></div></article>)}</div>}
    </div>
  </section>;
}
