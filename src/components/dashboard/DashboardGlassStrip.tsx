...
export function DashboardGlassStrip({ revenue, spend, leads, cpl, roas, forecast30, sales }: Props) {
  ...
  return <section className="sticky top-[calc(88px+env(safe-area-inset-top))] z-20 min-w-0 lg:top-[49px]" aria-label="Resumo fixo do Dashboard">
    <div className="dashboard-video-glass overflow-hidden rounded-[18px] border border-white/20 bg-background/72 shadow-[0_10px_36px_rgba(0,0,0,.18)] backdrop-blur-2xl supports-[backdrop-filter]:bg-background/58 dark:border-white/[.11] dark:bg-[#050506]/92 dark:shadow-[0_10px_38px_rgba(0,0,0,.42),inset_0_1px_0_rgba(255,255,255,.035)]">
      ...
      {!compact && <div className="dashboard-glass-metrics grid min-w-0 grid-cols-2 gap-1.5 border-t border-border/40 bg-transparent p-1.5 md:grid-cols-7 md:gap-1.5 md:border-t-0 md:p-2">{metrics.map((metric) => <article key={metric.label} className="group flex min-h-[66px] min-w-0 cursor-default items-center gap-1.5 rounded-xl border border-border/65 bg-gradient-to-br from-background/92 via-background/78 to-primary/[.035] px-1.5 py-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,.035)] transition duration-200 hover:-translate-y-px hover:border-primary/35 hover:shadow-[0_10px_28px_-20px_rgba(var(--brand-accent-rgb),.7),inset_0_1px_0_rgba(255,255,255,.055)] dark:border-white/[.07] dark:from-[#0c0c0d]/96 dark:via-[#080809]/92 dark:to-primary/[.055]" title={metricDescription(metric.label)}><span className={cn("grid h-8 w-8 shrink-0 place-items-center rounded-[10px] border border-primary/15 bg-primary/[.09] text-primary shadow-[inset_0_1px_0_rgba(255,255,255,.06)] transition group-hover:border-primary/30 group-hover:bg-primary/[.14] [&_svg]:h-3.5 [&_svg]:w-3.5", metric.tone === "good" && "border-emerald-500/20 bg-emerald-500/10 text-emerald-500")}>{metric.icon}</span><div className="min-w-0 flex-1"><p className="whitespace-nowrap text-[6.5px] font-black uppercase leading-tight tracking-[.035em] text-muted-foreground" title={metric.label}>{metric.label}</p><p className="mt-1 whitespace-nowrap text-[12px] font-black leading-none tabular-nums" title={metric.value}>{metric.value}</p></div></article>)}</div>}
    </div>
  </section>;
}
