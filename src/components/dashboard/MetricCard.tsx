...
export function MetricCard({ title, value, variation, icon, prefix, suffix, decimals = 2, colorByValue, tooltip }: MetricCardProps) {
  ...
  return (
    <Card className="dashboard-terra-card gd-metric-card group min-w-0 cursor-default overflow-hidden" title={description} aria-label={`${title}. ${description}`}>
      <CardContent className="gd-metric-card-content flex min-h-[96px] min-w-0 flex-col justify-center p-3 sm:min-h-[104px] sm:p-4 xl:min-h-[112px] xl:p-5">
        <div className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
          <div className="flex min-w-0 flex-col items-start justify-center gap-1 text-left">
            <p className="line-clamp-2 max-w-full break-words text-[9px] font-bold uppercase leading-[1.18] tracking-[.08em] text-muted-foreground sm:text-[10px] xl:text-xs" title={title}>{title}</p>
        ...
        {variation !== undefined && (
          <div className={cn("mt-2 flex w-full min-w-0 items-center gap-1 overflow-hidden text-[10px] font-medium sm:text-xs", isPositive ? "text-emerald-600" : "text-red-500")}>
