    ...
    <Card className="h-full flex flex-col">
      <CardHeader className="pb-2 flex-shrink-0">
        ...
        <p className="text-xs text-muted-foreground">
          Veja quais dias da semana geram mais conversões no período selecionado.
        </p>
      </CardHeader>
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={byWeekday} margin={{ top: 24, right: 16, left: 0, bottom: 5 }}>
              ...
              <Tooltip formatter={(v: any) => [fmt(Number(v)), "Conversões"]} />
              <Bar dataKey="leads" fill="hsl(217, 91%, 60%)" radius={[6, 6, 0, 0]} animationDuration={600}>
                <LabelList dataKey="leads" position="top" style={{ fontSize: 12, fontWeight: 600, fill: "hsl(var(--foreground))" }} formatter={(v: any) => fmt(Number(v))} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )}
