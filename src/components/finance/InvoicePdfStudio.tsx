...
import { FileDown, FileText, Printer, RotateCcw, ShieldAlert } from "lucide-react";
...
export function InvoicePdfStudio({ companies, sales }: { companies: Company[]; sales: Sale[] }) {
  ...
  return <section className="gd-panel overflow-hidden">
    <div className="flex flex-col gap-3 border-b border-border p-5 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><FileText className="h-5 w-5" /></span><div><h2 className="font-black">Nota fiscal em PDF</h2><p className="mt-1 text-xs text-muted-foreground">Preencha manualmente ou comece por uma empresa e uma venda já registradas.</p></div></div><Button onClick={printPdf}><Printer className="mr-2 h-4 w-4" />Gerar PDF</Button></div>
    <div className="grid gap-4 p-5 xl:grid-cols-[minmax(0,1fr)_300px]"><div className="grid gap-4 sm:grid-cols-2"><Field label="Empresa emitente"><Select value={companyId} onValueChange={selectCompany}><SelectTrigger><SelectValue placeholder="Selecionar empresa" /></SelectTrigger><SelectContent>{companies.map((company) => <SelectItem key={company.id} value={company.id}>{company.legal_name || company.name}</SelectItem>)}</SelectContent></Select></Field><Field label="Preencher de uma venda"><Select value={saleId} onValueChange={selectSale}><SelectTrigger><SelectValue placeholder="Selecionar venda" /></SelectTrigger><SelectContent>{confirmedSales.map((sale) => <SelectItem key={sale.id} value={sale.id}>{sale.contact_name || sale.rd_product_name || "Venda sem contato"} · {money.format(sale.gross_revenue || sale.net_revenue)}</SelectItem>)}</SelectContent></Select></Field><Field label="Número"><Input value={invoice.number} onChange={(event) => update("number", event.target.value)} /></Field><Field label="Data de emissão"><Input type="date" value={invoice.issueDate} onChange={(event) => update("issueDate", event.target.value)} /></Field><Field label="Razão social / emitente"><Input value={invoice.issuerName} onChange={(event) => update("issuerName", event.target.value)} /></Field><Field label="CNPJ ou CPF do emitente"><Input value={invoice.issuerDocument} onChange={(event) => update("issuerDocument", event.target.value)} placeholder="00.000.000/0000-00" /></Field><div className="sm:col-span-2"><Field label="Endereço do emitente"><Input value={invoice.issuerAddress} onChange={(event) => update("issuerAddress", event.target.value)} /></Field></div><Field label="Tomador / cliente"><Input value={invoice.customerName} onChange={(event) => update("customerName", event.target.value)} /></Field><Field label="CPF ou CNPJ do cliente"><Input value={invoice.customerDocument} onChange={(event) => update("customerDocument", event.target.value)} /></Field><Field label="E-mail do cliente"><Input type="email" value={invoice.customerEmail} onChange={(event) => update("customerEmail", event.target.value)} /></Field><Field label="Código de serviço municipal"><Input value={invoice.serviceCode} onChange={(event) => update("serviceCode", event.target.value)} /></Field><div className="sm:col-span-2"><Field label="Endereço do cliente"><Input value={invoice.customerAddress} onChange={(event) => update("customerAddress", event.target.value)} /></Field></div><div className="sm:col-span-2"><Field label="Descrição do serviço"><Textarea value={invoice.description} onChange={(event) => update("description", event.target.value)} /></Field></div><Field label="Valor dos serviços"><Input inputMode="decimal" value={invoice.amount} onChange={(event) => update("amount", event.target.value)} placeholder="0,00" /></Field><Field label="Alíquota de tributos (%)"><Input inputMode="decimal" value={invoice.taxRate} onChange={(event) => update("taxRate", event.target.value)} placeholder="0,00" /></Field><div className="sm:col-span-2"><Field label="Observações"><Textarea value={invoice.notes} onChange={(event) => update("notes", event.target.value)} placeholder="Informações complementares" /></Field></div></div><aside className="rounded-2xl border border-primary/20 bg-primary/[.045] p-4"><p className="text-[10px] font-black uppercase tracking-[.16em] text-primary">Resumo da prévia</p><p className="mt-4 text-xs text-muted-foreground">Valor dos serviços</p><p className="text-xl font-black">{money.format(amount)}</p><p className="mt-3 text-xs text-muted-foreground">Tributos estimados</p><p className="font-bold">{money.format(tax)}</p><div className="mt-4 border-t border-primary/20 pt-4"><p className="text-xs text-muted-foreground">Total do documento</p><p className="text-2xl font-black text-primary">{money.format(total)}</p></div><div className="mt-5 flex gap-2 rounded-xl border border-amber-500/25 bg-amber-500/[.08] p-3 text-[11px] leading-relaxed text-muted-foreground"><ShieldAlert className="h-4 w-4 shrink-0 text-amber-500" />PDF de prévia. Para NFS-e oficial, conecte um provedor fiscal homologado e as credenciais da empresa.</div><Button variant="outline" size="sm" className="mt-4 w-full" onClick={() => { setCompanyId(""); setSaleId(""); setInvoice(initialInvoice()); }}><RotateCcw className="mr-2 h-3.5 w-3.5" />Limpar campos</Button><Button className="mt-2 w-full" onClick={printPdf}><FileDown className="mr-2 h-4 w-4" />Salvar como PDF</Button></aside></div>
  </section>;
}
