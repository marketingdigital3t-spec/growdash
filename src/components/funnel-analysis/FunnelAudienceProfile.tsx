...
type DealLocation = { lead_city?: string | null; lead_state?: string | null; contact_name?: string | null; contact_email?: string | null; rd_stage_name?: string | null; stage_bucket?: string | null; amount_total?: number | null; utm_source?: string | null; utm_medium?: string | null; utm_campaign?: string | null; utm_term?: string | null; utm_content?: string | null; utm_id?: string | null; lead_created_at?: string | null; custom_fields?: Record<string, unknown> | null };
...
export function rdField(deal: DealLocation, aliases: string[]): string | null {
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    const isAlias = normalized.includes(normalizedKey) || normalized.some((alias) => normalizedKey.endsWith(alias));
...
export function FunnelAudienceProfile({ deals, campaignIds, accountIds = [], startDate, endDate, metaLeads, data: providedData, loading: providedLoading = false }: { deals: DealLocation[]; campaignIds: string[]; accountIds?: string[]; startDate: Date; endDate: Date; metaLeads?: number; data?: AudienceProfileData; loading?: boolean }) {
  ...
  const coverage = metaTotal > 0 ? data.registrations / metaTotal * 100 : null;
  const rdOpportunityCount = deals.filter((deal) => deal.stage_bucket === "opportunity").length;
  const totalPipeline = deals.reduce((sum, deal) => sum + Number(deal.amount_total || 0), 0);
  const filledPersonal = deals.reduce((sum, deal) => sum + [deal.contact_name, deal.contact_email, deal.lead_city, deal.lead_state, deal.stage_bucket].filter(Boolean).length, 0);
  const totalPersonal = deals.length * 5;
  const [page, setPage] = useState(0);
  const [dealsCollapsed, setDealsCollapsed] = useState(false);
  const pageSize = 25;
  const pageRows = deals.slice(page * pageSize, (page + 1) * pageSize);
  const pages = Math.max(1, Math.ceil(deals.length / pageSize));
  const sourceFor = (type: BreakdownType) => type === "age" ? (rdSegments(deals, ["idade", "age", "faixa_etaria", "faixa etaria"]).length ? "RD agregado" : "Meta fallback") : type === "gender" ? (rdSegments(deals, ["sexo", "genero", "gênero", "gender"]).length ? "RD agregado" : "Meta fallback") : type === "publisher_platform" ? (rdSegments(deals, ["plataforma", "platform", "origem plataforma"]).length ? "RD agregado" : "Meta fallback") : "Meta agregado";
  const valueFor = (deal: DealLocation, aliases: string[]) => rdValue(deal, aliases);
  return <Card className="gd-analysis-card border-border/40 bg-card/60"><CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-start"><div><CardTitle className="text-base">Perfil do público e entrega</CardTitle><p className="mt-1 text-xs font-normal text-muted-foreground">RD é a fonte dos dados individuais do histórico sincronizado; Meta aparece apenas nos recortes agregados que o RD não fornece.</p></div><div className="flex shrink-0 gap-2 sm:ml-auto"><Button variant="outline" size="sm" onClick={toggleAll}><Minimize2 className="mr-2 h-4 w-4" />{allCollapsed ? "Expandir todos" : "Recolher todos"}</Button><Button variant="outline" size="sm" onClick={() => downloadAudienceReport(data, startDate, endDate)}><Download className="mr-2 h-4 w-4" />Baixar PDF</Button></div></div></CardHeader><CardContent className="space-y-4"><div className="rounded-lg border border-primary/20 bg-primary/[0.035] px-3 py-2 text-[11px] text-muted-foreground">Meta: <b className="text-foreground">{metaTotal.toLocaleString("pt-BR")} aquisições</b> · RD: <b className="text-foreground">{data.registrations.toLocaleString("pt-BR")} cadastros</b> · Oportunidades RD: <b className="text-foreground">{rdOpportunityCount.toLocaleString("pt-BR")}</b> · Em negociação: <b className="text-foreground">{formatAmount(totalPipeline)}</b>{coverage !== null && <> · cobertura RD/Meta: <b className="text-foreground">{coverage.toFixed(1)}%</b></>}. Preenchimento dos dados pessoais: <b className="text-foreground">{totalPersonal ? (filledPersonal / totalPersonal * 100).toFixed(1) : "0.0"}%</b>.</div>{errorMessage && <div role="alert" className="rounded-lg border border-red-500/30 bg-red-500/5 px-3 py-2 text-xs text-red-300">Falha ao carregar detalhamentos da Meta: {errorMessage}</div>}<section className="rounded-lg border border-border/60 p-4"><button type="button" onClick={() => setCollapsed((current) => ({ ...current, locations: !current.locations }))} aria-expanded={!collapsed.locations} className="flex w-full items-start justify-between gap-3 text-left"><span><h3 className="text-sm font-semibold">Localização dos cadastros <span className="text-xs font-normal text-muted-foreground">· RD individual</span></h3></span><ChevronDown className={`mt-1 h-4 w-4 shrink-0 text-muted-foreground transition-transform ${collapsed.locations ? "-rotate-90" : ""}`} /></button>{!collapsed.locations && <><div className="mt-3 flex flex-wrap gap-x-4 gap-y-2 text-xs">{data.locations.map((row) => <span key={row.key}><b>{row.key}</b> · {row.leads.toLocaleString("pt-BR")}</span>)}</div><p className="mt-3 text-[10px] text-muted-foreground">{data.registrations.toLocaleString("pt-BR")} cadastros no histórico sincronizado · {data.registrations ? `${(data.locatedRegistrations / data.registrations * 100).toFixed(1)}% com cidade ou estado informado` : "sem cadastros sincronizados"}.</p></>}</section><section className="rounded-lg border border-border/60 p-4"><button type="button" onClick={() => setDealsCollapsed((current) => !current)} aria-expanded={!dealsCollapsed} className="flex w-full items-start justify-between gap-2 text-left"><span><h3 className="text-sm font-semibold">Leads e negociações recebidos <span className="text-xs font-normal text-muted-foreground">· RD individual</span></h3><p className="mt-1 text-xs text-muted-foreground">Nome, cidade, UF, idade, sexo, plataforma, etapa, valor e UTMs exatamente como persistidos no CRM.</p></span><ChevronDown className={`mt-1 h-4 w-4 shrink-0 transition-transform ${dealsCollapsed ? "-rotate-90" : ""}`} /></button>{!dealsCollapsed && <><div className="mt-3 flex items-center justify-end"><span className="text-xs text-muted-foreground">Página {page + 1}/{pages}</span></div><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[1650px] text-left text-xs"><thead className="border-b text-[10px] uppercase tracking-wide text-muted-foreground"><tr><th className="pb-2 pr-3">Nome</th><th className="pb-2 pr-3">Cidade/UF</th><th className="pb-2 pr-3">Idade</th><th className="pb-2 pr-3">Sexo</th><th className="pb-2 pr-3">Plataforma</th><th className="pb-2 pr-3">Etapa</th><th className="pb-2 pr-3 text-right">Valor</th><th className="pb-2 pr-3">UTM source</th><th className="pb-2 pr-3">Campanha</th><th className="pb-2 pr-3">Conjunto</th><th className="pb-2 pr-3">Criativo</th><th className="pb-2">Entrada</th></tr></thead><tbody>{pageRows.map((deal, index) => <tr key={`${deal.contact_email || deal.contact_name || "lead"}-${deal.lead_created_at || index}`} className="border-b border-border/40"><td className="max-w-44 truncate py-2 pr-3 font-medium" title={deal.contact_name || "Não informado"}>{deal.contact_name || deal.contact_email || "Não informado"}</td><td className="py-2 pr-3">{[deal.lead_city, deal.lead_state].filter(Boolean).join(" · ") || "Não informado"}</td><td className="py-2 pr-3">{valueFor(deal, ["idade", "age", "faixa_etaria", "faixa etaria"])}</td><td className="py-2 pr-3">{valueFor(deal, ["sexo", "genero", "gênero", "gender"])}</td><td className="py-2 pr-3">{valueFor(deal, ["plataforma", "platform", "origem plataforma"])}</td><td className="py-2 pr-3">{deal.rd_stage_name || deal.stage_bucket || "Não informado"}</td><td className="py-2 pr-3 text-right tabular-nums">{formatAmount(deal.amount_total)}</td><td className="py-2 pr-3">{deal.utm_source || "Não informado"}</td><td className="max-w-48 truncate py-2 pr-3" title={deal.utm_campaign || "Não informado"}>{deal.utm_campaign || "Não informado"}</td><td className="max-w-48 truncate py-2 pr-3" title={deal.utm_term || "Não informado"}>{deal.utm_term || "Não informado"}</td><td className="max-w-48 truncate py-2 pr-3" title={deal.utm_content || "Não informado"}>{deal.utm_content || "Não informado"}</td><td className="py-2">{deal.lead_created_at ? format(new Date(deal.lead_created_at), "dd/MM/yyyy HH:mm") : "Não informado"}</td></tr>)}</tbody></table></div><div className="mt-3 flex justify-end gap-2"><Button size="sm" variant="outline" onClick={() => setPage((current) => Math.max(0, current - 1))} disabled={page === 0}>Anterior</Button><Button size="sm" variant="outline" onClick={() => setPage((current) => Math.min(pages - 1, current + 1))} disabled={page >= pages - 1}>Próxima</Button></div></>}</section><div className="grid gap-4 xl:grid-cols-2">{audienceBreakdowns.map((item) => <MetaBreakdown key={item.type} {...item} rows={data.segments[item.type]} source={sourceFor(item.type)} loading={loading} collapsed={!!collapsed[item.type]} onToggle={() => setCollapsed((current) => ({ ...current, [item.type]: !current[item.type] }))} />)}</div></CardContent></Card>;
}
