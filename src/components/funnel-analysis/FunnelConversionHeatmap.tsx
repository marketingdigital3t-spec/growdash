...
import { rdDealWonDate, type RDDeal } from "@/hooks/useRDDeals";
...
export function FunnelConversionHeatmap({ closedDeals }: { closedDeals: RDDeal[] }) {
  ...
  closedDeals.forEach((deal) => { const wonDate = rdDealWonDate(deal); if (!wonDate) return; const date = new Date(wonDate); if (!Number.isNaN(date.getTime())) matrix.set(`${date.getDay()}-${periodOf(date)}`, (matrix.get(`${date.getDay()}-${periodOf(date)}`) ?? 0) + 1); });
