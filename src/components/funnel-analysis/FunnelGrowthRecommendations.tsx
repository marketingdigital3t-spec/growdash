...
export function FunnelGrowthRecommendations({ analytics, media }: { analytics: FunnelAnalytics; media: FunnelMediaMetrics }) {
  ...
  return <Card className="gd-analysis-card border-border/60 bg-card/70"><CardHeader className="pb-2"><CardTitle className="flex items-center gap-2 text-base"><Lightbulb className="h-4 w-4 text-foreground" />Dicas para reduzir custos e aumentar faturamento</CardTitle><p className="text-xs text-muted-foreground">Prioridades calculadas apenas com funil, vendas confirmadas e mídia do recorte selecionado. Margem líquida, impostos, notas, reembolsos e custos operacionais não têm fonte estruturada aqui.</p></CardHeader><CardContent className="grid gap-2 md:grid-cols-2">{recommendations.map((item) => { const Icon = item.tone === "warning" ? TriangleAlert : item.tone === "ok" ? CheckCircle2 : Lightbulb; return <div key={item.title} className="rounded-lg border border-border/50 bg-background/40 p-3"><div className="flex items-start gap-2"><Icon className="mt-0.5 h-4 w-4 shrink-0 text-foreground" /><div><h3 className="text-xs font-bold">{item.title}</h3><p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">{item.detail}</p></div></div></div>; })}</CardContent></Card>;
}
