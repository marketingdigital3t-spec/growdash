  ...
  return (
    <Card className="gd-analysis-card min-w-0 bg-card/60 border-border/40">
      <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
        ...
        <Badge variant="outline" className="shrink-0 tabular-nums">{attributedSales.toLocaleString("pt-BR")} de {totalSales.toLocaleString("pt-BR")} atribuídas</Badge>
      </CardHeader>
              ...
              <tbody className="divide-y divide-border/45">
                {rows.map((row) => {
                  ...
                  return <tr key={`${row.campaign}-${row.creative}`} className="transition-colors hover:bg-muted/25">
                    ...
                    <td className="px-4 py-3"><span className={untracked ? "inline-flex items-center gap-1 text-amber-400" : row.sales >= 2 ? "inline-flex items-center gap-1 text-emerald-400" : "inline-flex items-center gap-1 text-muted-foreground"}>{untracked ? <TriangleAlert className="h-3.5 w-3.5" /> : <TrendingUp className="h-3.5 w-3.5" />}{decision}</span></td>
