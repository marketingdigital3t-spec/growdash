...
export function SalesWebhookGatewayCard() {
  ...
  const [connection, setConnection] = useState<{ endpoint: string; secret?: string; header: string } | null>(null);
  ...
  return <section className="gd-panel overflow-hidden">
    <div className="flex flex-col gap-3 border-b border-border p-5 sm:flex-row sm:items-center"><span className="grid h-11 w-11 place-items-center rounded-xl bg-primary/10 text-primary"><Webhook className="h-5 w-5" /></span><div className="grow"><h2 className="font-black">Webhooks de vendas</h2><p className="text-xs text-muted-foreground">Receba vendas, reembolsos e chargebacks em tempo real sem duplicar faturamento.</p></div></div>
    <div className="space-y-4 p-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><label className="text-sm font-semibold" htmlFor="sales-webhook-provider">Plataforma</label><select id="sales-webhook-provider" value={provider} onChange={(event) => setProvider(event.target.value as typeof provider)} className="h-10 rounded-md border border-input bg-background px-3 text-sm sm:min-w-56">{platforms.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select><Button className="sm:ml-auto" onClick={create} disabled={pending}>{pending ? <RefreshCw className="mr-2 h-4 w-4 animate-spin" /> : <Link2 className="mr-2 h-4 w-4" />}{connection ? "Gerar novo segredo" : "Gerar webhook"}</Button></div>
      {connection && <div className="space-y-3 rounded-xl border border-primary/25 bg-primary/[.04] p-4 text-xs"><p className="font-semibold">Configure na plataforma: envie eventos de compra aprovada, reembolso e chargeback.</p><CopyValue label="URL do webhook" value={connection.endpoint} onCopy={copy} /><CopyValue label={`Cabeçalho ${connection.header}`} value={connection.secret || "Segredo mantido; gere um novo para visualizar."} onCopy={copy} /><p className="text-muted-foreground">O segredo é mostrado somente ao gerar ou rotacionar. A URL sozinha não autoriza vendas.</p></div>}
    </div>
  </section>;
}
