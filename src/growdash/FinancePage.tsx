...
export default function FinancePage() {
  ...
  const [includeMetaTax, setIncludeMetaTax] = useState(false);
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <div className="mb-4 flex items-center justify-end gap-3 rounded-xl border border-border bg-card px-4 py-3"><div className="text-right"><b className="block text-xs">Incluir taxa/imposto Meta</b><span className="text-[10px] text-muted-foreground">Simulação de 12%; o dado bruto permanece intacto.</span></div><Switch checked={includeMetaTax} onCheckedChange={setIncludeMetaTax} /></div>
      ...
      <Tabs defaultValue="dashboard" className="space-y-4">
        ...
        <TabsContent value="dre"><section className="gd-panel overflow-hidden"><PanelTitle icon={<ReceiptText />} title="DRE gerencial" subtitle="Regime de competência no período selecionado, com custos rastreáveis." /><DreRow label="Receita líquida de vendas" value={aggregateSales(visibleSales).totalNet} /><DreRow label="Outras receitas" value={otherRevenue} /><DreRow label="Receita total" value={totalRevenue} strong /><DreRow label="(-) Mídia Meta — dado original" value={-spend} />{includeMetaTax && <DreRow label="(-) Taxa/imposto Meta simulado (12%)" value={-metaTax} />}<DreRow label="(-) Despesas operacionais" value={-expenses} /><DreRow label="Resultado operacional" value={result} strong highlight /><DreRow label="Margem operacional" value={margin} percentage /></section></TabsContent>
        ...
        <TabsContent value="traffic" className="space-y-4"><ChartPanel title="Investimento semanal por todas as contas filtradas"><ResponsiveContainer width="100%" height={310}><BarChart data={weeklyInvestment}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="label" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} tickFormatter={compactCurrency} /><Tooltip formatter={currencyTooltip} /><Bar dataKey="investment" name="Investimento" fill="hsl(var(--primary))" radius={[5,5,0,0]} /></BarChart></ResponsiveContainer></ChartPanel><section className="gd-panel overflow-hidden"><PanelTitle icon={<TrendingUp />} title="Investimento mensal" subtitle={`${includeMetaTax ? "Inclui simulação de taxa Meta de 12%" : "Valores originais sincronizados da Meta"}.`} /><TrafficMonthlyTable rows={monthlyHistory} /></section></TabsContent>
