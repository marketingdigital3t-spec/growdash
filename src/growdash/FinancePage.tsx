...
import { addMonths, differenceInCalendarDays, eachDayOfInterval, eachMonthOfInterval, endOfMonth, format, startOfMonth, startOfWeek, subMonths } from "date-fns";
...
export default function FinancePage() {
  ...
  const { data: financialTransactions = [], isLoading: loadingTransactions } = useQuery({
    ...
    queryFn: async (): Promise<FinancialTransaction[]> => {
      ...
      const { data, error } = await request;
      if (error) {
        if (error.code === "42P01" || /financial_transactions|schema cache|does not exist/i.test(error.message)) return [];
        throw error;
      }
      return data ?? [];
    },
  ...
  const cashMovements = useMemo(() => {
    if (financialTransactions.length) return financialTransactions.map((item) => ({
      id: item.id,
      date: format(new Date(item.occurred_at), "yyyy-MM-dd"),
      description: item.description || "Movimentação bancária",
      amount: Number(item.amount || 0),
      type: /credit|income|revenue|receipt|entrada/i.test(item.transaction_type) ? "in" as const : "out" as const,
      source: "Conta conectada",
    }));
    return entries.filter((item) => item.status === "paid").map((item) => ({
      id: item.id,
      date: item.paid_at || item.competence_date,
      description: item.description,
      amount: Number(item.amount || 0),
      type: item.entry_type === "revenue" ? "in" as const : "out" as const,
      source: "Lançamento manual",
    }));
  }, [entries, financialTransactions]);
  // A seleção "Máximo" pode cobrir anos. Exibir milhares de pontos diários
  // derruba o gráfico e torna toda a aba financeira lenta; acima de 92 dias,
  // os mesmos lançamentos são consolidados por mês sem perder valores.
  const cashFlowIsMonthly = differenceInCalendarDays(endDate, startDate) > 92;
  const cashFlow = useMemo(() => {
    const bucketKey = (value: Date | string) => {
      const date = typeof value === "string" ? new Date(`${value.slice(0, 10)}T12:00:00`) : value;
      return format(date, cashFlowIsMonthly ? "yyyy-MM" : "yyyy-MM-dd");
    };
    const movementsByDay = new Map<string, typeof cashMovements>();
    cashMovements.forEach((movement) => {
      const key = bucketKey(movement.date);
      movementsByDay.set(key, [...(movementsByDay.get(key) ?? []), movement]);
    });
    const netInPeriod = cashMovements.reduce((sum, item) => sum + (item.type === "in" ? item.amount : -item.amount), 0);
    // The Open Finance balance is the current real balance. Reversing the
    // displayed period gives a useful opening balance without fabricating it.
    const opening = financialTransactions.length ? connectedBalance - netInPeriod : 0;
    let runningBalance = opening;
    const buckets = cashFlowIsMonthly
      ? eachMonthOfInterval({ start: startDate, end: endDate })
      : eachDayOfInterval({ start: startDate, end: endDate });
    return buckets.map((day) => {
      const date = bucketKey(day);
      const movements = movementsByDay.get(date) ?? [];
      const inflow = movements.filter((item) => item.type === "in").reduce((sum, item) => sum + item.amount, 0);
      const outflow = movements.filter((item) => item.type === "out").reduce((sum, item) => sum + item.amount, 0);
      runningBalance += inflow - outflow;
      return { date, label: format(day, cashFlowIsMonthly ? "MM/yy" : "dd/MM"), inflow, outflow, net: inflow - outflow, balance: runningBalance, movements };
    });
  }, [cashFlowIsMonthly, cashMovements, connectedBalance, endDate, financialTransactions.length, startDate]);
  const cashIn = cashMovements.filter((item) => item.type === "in").reduce((sum, item) => sum + item.amount, 0);
  const cashOut = cashMovements.filter((item) => item.type === "out").reduce((sum, item) => sum + item.amount, 0);
  const pendingIn = entries.filter((item) => item.entry_type === "revenue" && item.status !== "paid" && item.status !== "canceled").reduce((sum, item) => sum + Number(item.amount), 0);
  const pendingOut = entries.filter((item) => item.entry_type === "expense" && item.status !== "paid" && item.status !== "canceled").reduce((sum, item) => sum + Number(item.amount), 0);
  const horizons = [
  ...
  return (
    <div className="gd-module-shell mx-auto w-full max-w-[1920px] space-y-5">
      ...
      <Tabs defaultValue="dre" className="space-y-4">
        <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto bg-muted/70 p-1"><TabsTrigger value="dre">DRE</TabsTrigger><TabsTrigger value="cash-flow">Fluxo de caixa</TabsTrigger><TabsTrigger value="dashboard">Dashboard</TabsTrigger><TabsTrigger value="forecast">Previsão</TabsTrigger><TabsTrigger value="accounts">Por conta</TabsTrigger><TabsTrigger value="traffic">Investimento em tráfego</TabsTrigger><TabsTrigger value="entries">Lançamentos</TabsTrigger><TabsTrigger value="invoices">Notas fiscais</TabsTrigger><TabsTrigger value="cards">Cartões</TabsTrigger><TabsTrigger value="companies">Empresas</TabsTrigger></TabsList>

        <TabsContent value="dre" className="grid gap-4 xl:grid-cols-2">
          <section className="gd-panel overflow-hidden">
            ...
            <DreRow label="(−) Materiais" value={-materials} />
          ...
          <section className="gd-panel overflow-hidden">
            <PanelTitle icon={<Building2 />} title="Resultado por empresa / conta" subtitle="Receita líquida atribuída menos mídia e imposto por conta conectada." />
            <div className="divide-y divide-border">{rows.map((row) => { const cost = row.spend * (includeMetaTax ? 1 + metaTaxRate : 1); const profit = row.revenue - cost; return <div key={row.account.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><b>{row.account.name}</b><p className="mt-1 text-[10px] text-muted-foreground">Meta Ads · {segment === "saas" ? "SaaS" : "Infoproduto"}</p></div><b className={profit < 0 ? "text-rose-500" : "text-emerald-500"}>{brl.format(profit)}</b></div><div className="mt-4 grid grid-cols-2 gap-2 text-[10px] sm:grid-cols-4"><span>Receita líquida<b className="mt-1 block text-sm">{brl.format(row.revenue)}</b></span><span>Tráfego<b className="mt-1 block text-sm">{brl.format(cost)}</b></span><span>Saldo em conta<b className="mt-1 block text-sm text-primary">{brl.format(row.balance)}</b></span><span>ROAS<b className="mt-1 block text-sm">{row.roas.toFixed(2)}x</b></span></div></div>; })}{!rows.length && <div className="grid min-h-72 place-items-center p-8 text-center"><div><ReceiptText className="mx-auto h-9 w-9 text-muted-foreground/50" /><p className="mt-3 text-xs text-muted-foreground">Sem dados no período para esta unidade.</p></div></div>}</div>
          </section>
        </TabsContent>

        <TabsContent value="cash-flow" className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Entradas realizadas" value={brl.format(cashIn)} change={financialTransactions.length ? "contas conectadas" : "lançamentos recebidos"} emphasis /><MetricCard label="Saídas realizadas" value={brl.format(cashOut)} change={financialTransactions.length ? "contas conectadas" : "lançamentos pagos"} /><MetricCard label="Fluxo líquido" value={brl.format(cashIn - cashOut)} change="entradas − saídas" /><MetricCard label="Saldo final" value={brl.format(cashFlow.at(-1)?.balance ?? 0)} change={financialTransactions.length ? "saldo conciliado" : "cadastre/conecte uma conta"} /></div><section className="gd-panel overflow-hidden"><PanelTitle icon={<CircleDollarSign />} title={cashFlowIsMonthly ? "Fluxo de caixa mensal" : "Fluxo de caixa diário"} subtitle={financialTransactions.length ? "Movimentações de contas conectadas; o saldo inicial é calculado a partir do saldo bancário atual." : "Sem extrato conectado: exibindo somente lançamentos marcados como pagos/recebidos. Pendências não entram no caixa."} /><div className="p-4 sm:p-5"><ResponsiveContainer width="100%" height={300}><ComposedChart data={cashFlow}><defs><linearGradient id="cashBalanceFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#34d399" stopOpacity={.28}/><stop offset="95%" stopColor="#34d399" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="label" minTickGap={24} tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} tickFormatter={compactCurrency} /><Tooltip formatter={currencyTooltip} /><Legend /><Area type="monotone" dataKey="balance" name="Saldo acumulado" stroke="#34d399" fill="url(#cashBalanceFill)" strokeWidth={2} /><Bar dataKey="inflow" name="Entradas" fill="#34d399" radius={[3,3,0,0]} /><Bar dataKey="outflow" name="Saídas" fill="#fb7185" radius={[3,3,0,0]} /></ComposedChart></ResponsiveContainer></div></section><div className="grid gap-4 xl:grid-cols-2"><section className="gd-panel overflow-hidden"><PanelTitle icon={<Landmark />} title="Movimentações do período" subtitle={`${cashMovements.length} lançamento(s) efetivamente realizado(s).`} /><CashMovementTable movements={cashMovements} /></section><section className="gd-panel overflow-hidden"><PanelTitle icon={<ReceiptText />} title="A receber e a pagar" subtitle="Compromissos pendentes — fora do saldo realizado até a baixa." /><div className="grid gap-3 p-5 sm:grid-cols-2"><div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4"><p className="text-[10px] font-black uppercase tracking-[.12em] text-emerald-600">A receber</p><b className="mt-2 block text-2xl text-emerald-500">{brl.format(pendingIn)}</b><p className="mt-1 text-xs text-muted-foreground">Lançamentos ainda não recebidos</p></div><div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-4"><p className="text-[10px] font-black uppercase tracking-[.12em] text-rose-600">A pagar</p><b className="mt-2 block text-2xl text-rose-500">{brl.format(pendingOut)}</b><p className="mt-1 text-xs text-muted-foreground">Lançamentos ainda não pagos</p></div></div></section></div></TabsContent>

        <TabsContent value="dashboard" className="space-y-4"><div className="overflow-x-auto pb-2"><div className="grid min-w-[1200px] grid-cols-6 gap-3"><MetricCard label="Receita total" value={isLoading ? "Carregando…" : brl.format(totalRevenue)} change="vendas + receitas" emphasis /><MetricCard label="Despesas totais" value={isLoading ? "Carregando…" : brl.format(adjustedSpend + expenses)} change="mídia + operação" /><MetricCard label="Lucro" value={isLoading ? "Carregando…" : brl.format(result)} change={result < 0 ? "revisar" : "positivo"} /><MetricCard label="Margem" value={isLoading ? "Carregando…" : pct.format(margin)} change="lucro / receita" /><MetricCard label="Cartões e bancos" value={brl.format(connectedBalance)} change={`${financialAccounts.length} conectado(s)`} /><MetricCard label="Tráfego pago" value={isLoading ? "Carregando…" : brl.format(adjustedSpend)} change={includeMetaTax ? "com taxa" : "dado Meta"} /></div></div><ChartPanel title="Receita, despesas e resultado — últimos 12 meses"><ResponsiveContainer width="100%" height={300}><AreaChart data={monthlyHistory}><defs><linearGradient id="revenueFill" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#34d399" stopOpacity={0.32}/><stop offset="95%" stopColor="#34d399" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="label" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} tickFormatter={compactCurrency} /><Tooltip formatter={currencyTooltip} /><Legend /><Area type="monotone" dataKey="revenue" name="Receita" stroke="#34d399" fill="url(#revenueFill)" /><Line type="monotone" dataKey="expense" name="Despesa" stroke="#fb7185" strokeWidth={2} /><Line type="monotone" dataKey="result" name="Resultado" stroke="#34d399" strokeWidth={2} /></AreaChart></ResponsiveContainer></ChartPanel><MonthlyTable rows={monthlyHistory} /></TabsContent>

        <TabsContent value="forecast" className="space-y-4"><div className="rounded-2xl border border-primary/25 bg-primary/5 p-5"><div className="flex gap-3"><Sparkles className="h-5 w-5 text-primary" /><div><b>Previsão por média móvel e tendência dos últimos 12 meses</b><p className="mt-1 text-xs text-muted-foreground">Cenário orientativo, não promessa. Base pondera média histórica, últimos seis meses e tendência; otimista usa +10% receita/−3% despesa e pessimista −10%/+8%.</p></div></div></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><MetricCard label="Caixa gerado (12m)" value={brl.format(cash12m)} change="receita − despesa" emphasis /><MetricCard label="Receita média/mês" value={brl.format(avgRevenue)} change="12 meses" /><MetricCard label="Despesa média/mês" value={brl.format(avgExpense)} change="12 meses" /><MetricCard label="Runway estimado" value={avgExpense > avgRevenue && connectedBalance > 0 ? `${Math.floor(connectedBalance / (avgExpense - avgRevenue))} meses` : "Operação sustentável"} change="com saldo conectado" /></div><section className="gd-panel p-5"><h2 className="font-black">Projeção de resultado por horizonte</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">{horizons.map((item) => <div key={item.label} className="rounded-xl border border-border p-4"><b className="text-xs">{item.label}</b><p className={`mt-2 text-xl font-black ${item.base < 0 ? "text-rose-500" : "text-emerald-500"}`}>{brl.format(item.base)}</p><div className="mt-3 space-y-1 text-[10px] text-muted-foreground"><p>Receita <b className="float-right text-foreground">{brl.format(item.revenue)}</b></p><p>Despesa <b className="float-right text-foreground">{brl.format(item.expense)}</b></p><p>Otimista <b className="float-right text-emerald-500">{brl.format(item.optimistic)}</b></p><p>Pessimista <b className="float-right text-rose-500">{brl.format(item.pessimistic)}</b></p></div></div>)}</div></section><div className="grid gap-4 xl:grid-cols-2"><ChartPanel title="Receita e despesa — real x previsto"><ResponsiveContainer width="100%" height={290}><AreaChart data={chartForecast}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="label" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} tickFormatter={compactCurrency} /><Tooltip formatter={currencyTooltip} /><Legend /><Line dataKey="realRevenue" name="Receita real" stroke="#34d399" dot={false} /><Line dataKey="forecastRevenue" name="Receita prevista" stroke="#34d399" strokeDasharray="5 4" dot={false} /><Line dataKey="realExpense" name="Despesa real" stroke="#fb7185" dot={false} /><Line dataKey="forecastExpense" name="Despesa prevista" stroke="#fb7185" strokeDasharray="5 4" dot={false} /></AreaChart></ResponsiveContainer></ChartPanel><ChartPanel title="Resultado mensal projetado"><ResponsiveContainer width="100%" height={290}><BarChart data={forecast}><CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /><XAxis dataKey="label" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} tickFormatter={compactCurrency} /><ReferenceLine y={0} stroke="hsl(var(--foreground))" strokeOpacity={.45} /><Tooltip formatter={currencyTooltip} /><Bar dataKey="result" name="Resultado" radius={[4,4,0,0]}>{forecast.map((item) => <Cell key={item.label} fill={item.result < 0 ? "#fb7185" : "#34d399"} />)}</Bar></BarChart></ResponsiveContainer></ChartPanel></div></TabsContent>
        ...
        <TabsContent value="invoices"><InvoicePdfStudio companies={companies} sales={visibleSales} workspaceId={workspace?.id} userId={user?.id} /></TabsContent>
...
function ReconciliationMetric({ label, value, accent, strong }: { label: string; value: number; accent?: boolean; strong?: boolean }) { return <div className="rounded-xl border border-border bg-background/60 p-3"><p className="text-[9px] font-bold uppercase tracking-[.1em] text-muted-foreground">{label}</p><b className={`mt-1 block text-sm ${strong ? "text-base text-foreground" : accent ? "text-primary" : "text-foreground"}`}>{brl.format(value)}</b></div>; }
function AccountTable({ rows, loading }: { rows: any[]; loading: boolean }) { return <section className="gd-panel overflow-hidden"><PanelTitle icon={<TrendingUp />} title="Mídia x faturamento líquido" subtitle="Cruzamento por conta de anúncio." /><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[850px] text-left text-xs"><thead className="bg-muted/60 text-[10px] text-muted-foreground"><tr>{["Conta", "Plataforma", "Investimento", "Leads", "Vendas", "Faturamento líquido", "ROAS"].map((label) => <th key={label} className="px-4 py-3">{label}</th>)}</tr></thead><tbody className="divide-y divide-border">{rows.map((row) => <tr key={row.account.id}><td className="px-4 py-4 font-black">{row.account.name}</td><td className="px-4 py-4">Meta Ads</td><td className="px-4 py-4">{brl.format(row.spend)}</td><td className="px-4 py-4">{row.leads}</td><td className="px-4 py-4">{row.sales}</td><td className="px-4 py-4 font-black">{brl.format(row.revenue)}</td><td className="px-4 py-4 font-black text-emerald-500">{row.roas.toFixed(2)}x</td></tr>)}</tbody></table></div><div className="divide-y divide-border md:hidden">{rows.map((row) => <div key={row.account.id} className="p-4"><b className="text-sm">{row.account.name}</b><div className="mt-3 grid grid-cols-2 gap-2 text-xs"><span>Investimento <b className="block">{brl.format(row.spend)}</b></span><span>Faturamento líquido <b className="block">{brl.format(row.revenue)}</b></span><span>Leads <b className="block">{row.leads}</b></span><span>ROAS <b className="block text-emerald-500">{row.roas.toFixed(2)}x</b></span></div></div>)}</div>{!loading && !rows.length && <div className="p-10 text-center text-xs text-muted-foreground">Nenhuma conta conectada para esta unidade.</div>}</section>; }
function MonthlyTable({ rows }: { rows: { label: string; revenue: number; expense: number; result: number }[] }) { const [collapsed, setCollapsed] = useState(false); return <section className="gd-panel overflow-hidden"><header className="flex items-center justify-between border-b border-border px-4 py-2.5"><div><h3 className="text-xs font-black">Detalhamento mensal</h3><p className="text-[10px] text-muted-foreground">Entradas, saídas e resultado</p></div><Button type="button" size="sm" variant="ghost" className="h-7 px-2 text-[10px]" onClick={() => setCollapsed((value) => !value)} aria-expanded={!collapsed}>{collapsed ? "Expandir" : "Minimizar"}<ChevronDown className={`ml-1 h-3.5 w-3.5 transition-transform ${collapsed ? "-rotate-90" : ""}`} /></Button></header>{!collapsed && <div className="max-h-[260px] overflow-auto"><table className="w-full min-w-[620px] text-left text-[11px]"><thead className="sticky top-0 bg-muted/95 text-[10px] text-muted-foreground"><tr><th className="px-3 py-2">Mês</th><th className="px-3 py-2">Entradas</th><th className="px-3 py-2">Saídas</th><th className="px-3 py-2">Resultado</th><th className="px-3 py-2">Margem</th></tr></thead><tbody className="divide-y divide-border">{rows.map((row) => <tr key={row.label}><td className="px-3 py-2 font-black">{row.label}</td><td className="px-3 py-2 text-emerald-500">{brl.format(row.revenue)}</td><td className="px-3 py-2 text-rose-500">{brl.format(row.expense)}</td><td className={`px-3 py-2 font-black ${row.result < 0 ? "text-rose-500" : "text-emerald-500"}`}>{brl.format(row.result)}</td><td className="px-3 py-2">{row.revenue > 0 ? pct.format(row.result / row.revenue) : "—"}</td></tr>)}</tbody></table></div>}</section>; }
function CashMovementTable({ movements }: { movements: Array<{ id: string; date: string; description: string; amount: number; type: "in" | "out"; source: string }> }) { return <div className="max-h-[380px] overflow-y-auto divide-y divide-border">{movements.slice().sort((a, b) => b.date.localeCompare(a.date)).map((item) => <div key={item.id} className="flex items-center justify-between gap-3 px-5 py-3"><div className="min-w-0"><b className="block truncate text-xs">{item.description}</b><p className="mt-0.5 text-[10px] text-muted-foreground">{format(new Date(`${item.date.slice(0, 10)}T12:00:00`), "dd/MM/yyyy")} · {item.source}</p></div><b className={`shrink-0 text-xs ${item.type === "in" ? "text-emerald-500" : "text-rose-500"}`}>{item.type === "in" ? "+" : "−"}{brl.format(item.amount)}</b></div>)}{!movements.length && <div className="p-10 text-center text-xs text-muted-foreground">Nenhuma movimentação realizada no período selecionado.</div>}</div>; }
