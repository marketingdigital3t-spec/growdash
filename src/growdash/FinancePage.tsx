...
export default function FinancePage() {
  ...
  const rows = useMemo(() => accounts.map((account) => { const ai = insights.filter((item) => item.ad_account_id === account.id); const as = sales.filter((item) => item.ad_account_id === account.id); const accountSpend = ai.reduce((sum, item) => sum + Number(item.spend || 0), 0); const saleTotals = aggregateSales(as); const revenue = saleTotals.totalNet; return { account, spend: accountSpend, leads: ai.reduce((sum, item) => sum + Number(item.leads || 0), 0), sales: saleTotals.totalQuantity, revenue, roas: accountSpend > 0 ? revenue / accountSpend : 0 }; }), [accounts, insights, sales]);
