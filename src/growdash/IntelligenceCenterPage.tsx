...
export default function IntelligenceCenterPage() {
  ...
  return <div className="mx-auto w-full max-w-[1920px] space-y-4">
    ...
    <>
      ...
      {tab === "creative" && <section className="space-y-3">{intelligence.creativeFatigue.length ? intelligence.creativeFatigue.slice(0, 20).map((creative) => <article key={creative.adId} className="gd-panel grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_120px_120px_120px]"><div className="min-w-0"><b className="block truncate">{creative.name}</b><span className={cn("text-xs", creative.needsReplacement ? "text-red-400" : "text-emerald-400")}>{creative.needsReplacement ? "Substituição recomendada" : "Sem fadiga crítica"}</span></div><Value label="Fadiga" value={`${decimal.format(creative.fatigueScore)}%`} /><Value label="Frequência" value={decimal.format(creative.frequency)} /><Value label="CTR" value={`${decimal.format(creative.ctr)}%`} /></article>) : <article className="gd-panel p-6"><h2 className="font-black">Criativos sem métricas no período</h2><div className="mt-4 grid gap-3 sm:grid-cols-3"><Comparison label="Fadiga" value="0,00%" detail="sem variação" /><Comparison label="Frequência" value="0,00" detail="sem entrega" /><Comparison label="CTR" value="0,00%" detail="sem cliques" /></div></article>}</section>}
