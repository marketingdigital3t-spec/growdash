...
import { ArrowLeft, BriefcaseBusiness, CalendarDays, Check, ChevronLeft, ChevronRight, Columns3, Headphones, ImageUp, LayoutGrid, List, ListTodo, Megaphone, Pencil, Plus, Search, Trash2, X } from "lucide-react";
...
export default function KanbanPage() {
  ...
  if (activeBoard) {
    ...
    <CardDialog dialog={cardDialog} form={cardForm} lists={details.lists} saving={savingCard} workspaceId={workspace?.id} onUploadBanner={uploadCardBanner} setForm={setCardForm} onClose={() => setCardDialog(null)} onSave={() => void saveCard()} />
...
function CardDialog({ dialog, form, lists, saving, workspaceId, onUploadBanner, setForm, onClose, onSave }: { dialog: { listId: string; card?: KanbanCard } | null; form: { title: string; description: string; due_date: string; priority: string; list_id: string }; lists: KanbanList[]; saving: boolean; workspaceId?: string; onUploadBanner: (card: KanbanCard, file?: File) => Promise<void>; setForm: (form: { title: string; description: string; due_date: string; priority: string; list_id: string }) => void; onClose: () => void; onSave: () => void }) {
  return <Dialog open={!!dialog} onOpenChange={(open) => !open && !saving && onClose()}><DialogContent className="max-w-lg"><DialogHeader><DialogTitle>{dialog?.card ? "Editar cartão" : "Novo cartão"}</DialogTitle></DialogHeader><div className="space-y-3">{dialog?.card && <div className="overflow-hidden rounded-xl border border-border bg-muted/20">{dialog.card.banner_url && <img src={dialog.card.banner_url} alt="Banner atual do cartão" className="h-32 w-full object-cover" />}<label className="flex cursor-pointer items-center justify-center gap-2 p-3 text-xs font-bold hover:bg-muted/40"><ImageUp className="h-4 w-4" />{dialog.card.banner_url ? "Trocar imagem de referência" : "Adicionar imagem de referência"}<input type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" disabled={saving || !workspaceId} onChange={(event) => { void onUploadBanner(dialog.card, event.target.files?.[0]); event.currentTarget.value = ""; }} /></label></div>}<div><Label htmlFor="kanban-card-title">Título</Label><Input id="kanban-card-title" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} autoFocus disabled={saving} /></div><div><Label htmlFor="kanban-card-description">Descrição</Label><Textarea id="kanban-card-description" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} rows={4} disabled={saving} /></div><div className="grid grid-cols-2 gap-3"><div><Label htmlFor="kanban-card-due-date">Prazo</Label><Input id="kanban-card-due-date" type="date" value={form.due_date} onChange={(event) => setForm({ ...form, due_date: event.target.value })} disabled={saving} /></div><div><Label>Prioridade</Label><Select value={form.priority} onValueChange={(value) => setForm({ ...form, priority: value })} disabled={saving}><SelectTrigger aria-label="Prioridade do cartão"><SelectValue /></SelectTrigger><SelectContent>{PRIORITIES.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</Select></div></div>{dialog?.card && <div><Label>Mover para coluna</Label><Select value={form.list_id} onValueChange={(value) => setForm({ ...form, list_id: value })} disabled={saving}><SelectTrigger aria-label="Coluna do cartão"><SelectValue /></SelectTrigger><SelectContent>{lists.map((list) => <SelectItem key={list.id} value={list.id}>{list.name}</SelectItem>)}</SelectContent></Select></div>}</div><DialogFooter><Button variant="outline" onClick={onClose} disabled={saving}>Cancelar</Button><Button onClick={onSave} disabled={saving || !form.title.trim()}><Check className="mr-2 h-4 w-4" />{saving ? "Salvando…" : "Salvar cartão"}</Button></DialogFooter></DialogContent></Dialog>;
}
