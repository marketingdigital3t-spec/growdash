...
function KanbanColumn({ list, cards, onDrop, onAdd, onEdit, onDelete, onRename }: { list: KanbanList; cards: KanbanCard[]; onDrop: (cardId: string, targetPosition?: number) => void; onAdd: () => void; onEdit: (listId: string, card: KanbanCard) => void; onDelete: (card: KanbanCard) => void; onRename: (list: KanbanList, name: string) => Promise<void> }) {
  ...
  return <section className="gd-panel w-[300px] shrink-0 bg-muted/25 p-3" aria-label={`Coluna ${list.name}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const id = event.dataTransfer.getData("text/kanban-card"); if (id) onDrop(id, cards.length); }}><header className="mb-3 flex items-center justify-between gap-2"><div className="flex min-w-0 items-center gap-2"><Columns3 className="h-4 w-4 shrink-0 text-primary" />{editing ? <Input value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void save(); if (event.key === "Escape") cancel(); }} autoFocus disabled={saving} aria-label={`Nome da etapa ${list.name}`} className="h-8 min-w-0 text-sm font-black" /> : <h2 className="truncate text-sm font-black">{list.name}</h2>}<span className="rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-black text-primary">{cards.length}</span></div><div className="flex items-center gap-1">{editing ? <><Button variant="ghost" size="icon" className="h-8 w-8 text-emerald-600 hover:bg-emerald-500/10" onClick={() => void save()} disabled={saving || !name.trim()} aria-label="Salvar nome da etapa"><Check className="h-4 w-4" /></Button><Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground" onClick={cancel} disabled={saving} aria-label="Cancelar edição da etapa"><X className="h-4 w-4" /></Button></> : <><Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-primary" onClick={() => { setName(list.name); setEditing(true); }} aria-label={`Editar etapa ${list.name}`} title="Editar nome da etapa"><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" className="h-8 w-8" onClick={onAdd} aria-label={`Adicionar em ${list.name}`}><Plus className="h-4 w-4" /></Button></>}</div></header><div className="min-h-24 space-y-2" role="list">{cards.map((card, index) => <article key={card.id} role="listitem" aria-label={`Cartão ${card.title}`} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/kanban-card", card.id); }} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); const id = event.dataTransfer.getData("text/kanban-card"); if (id) onDrop(id, index); }} className="group cursor-grab rounded-xl border border-border bg-background p-3 shadow-sm active:cursor-grabbing"><div className="flex items-start gap-2"><button type="button" onClick={() => onEdit(list.id, card)} className="min-w-0 grow text-left"><b className="block break-words text-xs">{card.title}</b>{card.description && <p className="mt-1 line-clamp-2 text-[10px] text-muted-foreground">{card.description}</p>}</button><button type="button" onClick={() => onDelete(card)} className="shrink-0 rounded p-1 text-muted-foreground opacity-0 transition hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100 focus-visible:opacity-100" aria-label={`Excluir cartão ${card.title}`}><Trash2 className="h-3.5 w-3.5" /></button></div><div className="mt-3 flex items-center justify-between gap-2 text-[9px] text-muted-foreground"><PriorityBadge priority={card.priority} />{card.due_date && <span className="inline-flex items-center gap-1"><CalendarDays className="h-3 w-3" />{format(new Date(`${card.due_date}T12:00:00`), "dd/MM")}</span>}</div></article>)}{!cards.length && <div className="grid min-h-24 place-items-center rounded-xl border border-dashed border-border p-4 text-[10px] text-muted-foreground">Arraste cartões para cá</div>}</div><Button variant="ghost" className="mt-2 w-full justify-start text-xs font-bold text-foreground hover:bg-primary/10 hover:text-primary" onClick={onAdd}><Plus className="mr-1 h-3.5 w-3.5" />Adicionar cartão</Button></section>;
}
