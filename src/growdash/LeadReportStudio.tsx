...
function AudienceReportSection({ audience }: { audience: AudienceProfileData }) {
  return <section className="rounded-2xl border border-white/10 bg-white/[.035] p-5"><div><h2 className="font-black">Perfil do público e entrega</h2><p className="mt-1 text-[11px] text-white/45">Recortes agregados da entrega e das conversões atribuídas pela Meta; localização informada nos cadastros do RD.</p></div><div className="mt-5 grid gap-3 lg:grid-cols-2">{audienceBreakdowns.map((item) => { const rows = audience.segments[item.type]; const total = rows.reduce((sum, row) => sum + row.leads, 0); return <article key={item.type} className="rounded-xl border border-white/10 bg-black/10 p-4"><h3 className="text-sm font-black">{item.title} <span className="font-normal text-white/45">· Meta</span></h3>{rows.length ? <div className="mt-3 space-y-2">{rows.map((row) => <div key={row.key} className="flex items-center justify-between gap-3 text-xs"><span className="truncate text-white/65">{row.key}</span><b className="shrink-0">{integer.format(row.leads)} <small className="font-normal text-white/45">({total ? (row.leads / total * 100).toFixed(1) : "0.0"}%)</small></b></div>)}</div> : <p className="mt-3 text-xs text-white/40">Sem detalhamento sincronizado.</p>}</article>; })}<article className="rounded-xl border border-white/10 bg-black/10 p-4"><h3 className="text-sm font-black">Localização dos cadastros <span className="font-normal text-white/45">· RD</span></h3>{audience.locations.length ? <div className="mt-3 space-y-2">{audience.locations.map((row) => <div key={row.key} className="flex items-center justify-between gap-3 text-xs"><span className="truncate text-white/65">{row.key}</span><b className="shrink-0">{integer.format(row.leads)}</b></div>)}</div> : <p className="mt-3 text-xs text-white/40">Sem localização informada.</p>}<p className="mt-3 text-[10px] text-white/45">{integer.format(audience.registrations)} cadastros · {audience.registrations ? (audience.locatedRegistrations / audience.registrations * 100).toFixed(1) : "0.0"}% com cidade ou estado.</p></article></div></section>;
}
