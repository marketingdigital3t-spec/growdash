...
import { ArrowDownRight, ArrowLeft, ArrowUpRight, BarChart3, Check, ChevronDown, Copy, ExternalLink, ImageUp, Lightbulb, Link2, Presentation, Sparkles, Trash2, TrendingDown, TrendingUp } from "lucide-react";
...
export function ReportPresentation({ report, onClose, shareUrl, onCopy }: { report: ReportSnapshot; onClose?: () => void; shareUrl?: string; onCopy?: () => void }) {
  ...
  return <div className={cn(onClose && "fixed inset-0 z-[200]", "min-h-screen overflow-y-auto bg-[#050505] text-white")}>
    ...
    <main className="mx-auto max-w-[1800px] space-y-8 px-5 py-10 sm:px-10">
      ...
      {report.audience && <AudienceReportSection audience={report.audience} />}
...
function AudienceReportSection({ audience }: { audience: AudienceProfileData }) {
  const [collapsed, setCollapsed] = useState<Record<string, boolean>>({ age: true, gender: true, publisher_platform: true, platform_position: true, country: true, region: true, locations: true });
  const top = (type: (typeof audienceBreakdowns)[number]["type"]) => audience.segments[type]?.[0]?.key || "Sem dado";
  const toggle = (key: string) => setCollapsed((current) => ({ ...current, [key]: !current[key] }));
  const sections = [
    ...audienceBreakdowns.map((item) => ({ key: item.type, title: item.title, source: "Meta · agregado", description: item.description, rows: audience.segments[item.type] })),
    { key: "locations", title: "Localização dos cadastros", source: "RD · individual", description: "Cidade e estado informados nos cadastros sincronizados do RD.", rows: audience.locations.map((row) => ({ key: row.key, leads: row.leads, spend: 0, impressions: 0, clicks: 0 })) },
  ];
  return <section className="rounded-2xl border border-white/10 bg-white/[.035] p-4 sm:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-black">Perfil do público e entrega</h2><p className="mt-1 text-[11px] text-white/45">Quem está chegando, com a fonte identificada em cada bloco.</p></div><span className="rounded-full border border-white/10 bg-black/20 px-2.5 py-1 text-[10px] text-white/55">{integer.format(audience.registrations)} cadastros RD</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-xl border border-white/10 bg-black/15 px-3 py-2"><small className="block text-[10px] uppercase tracking-wide text-white/40">Faixa mais frequente</small><b className="mt-1 block truncate text-sm">{top("age")}</b><span className="text-[10px] text-white/40">Meta · agregado</span></div><div className="rounded-xl border border-white/10 bg-black/15 px-3 py-2"><small className="block text-[10px] uppercase tracking-wide text-white/40">Gênero predominante</small><b className="mt-1 block truncate text-sm">{top("gender")}</b><span className="text-[10px] text-white/40">Meta · agregado</span></div><div className="rounded-xl border border-white/10 bg-black/15 px-3 py-2"><small className="block text-[10px] uppercase tracking-wide text-white/40">Plataforma principal</small><b className="mt-1 block truncate text-sm">{top("publisher_platform")}</b><span className="text-[10px] text-white/40">Meta · agregado</span></div><div className="rounded-xl border border-white/10 bg-black/15 px-3 py-2"><small className="block text-[10px] uppercase tracking-wide text-white/40">Local mais informado</small><b className="mt-1 block truncate text-sm">{audience.locations[0]?.key || "Sem dado"}</b><span className="text-[10px] text-white/40">RD · individual</span></div></div><div className="mt-3 grid gap-2 lg:grid-cols-2">{sections.map((section) => { const rows = section.rows; const total = rows.reduce((sum, row) => sum + row.leads, 0); const isCollapsed = !!collapsed[section.key]; return <article key={section.key} className="rounded-xl border border-white/10 bg-black/10"><button type="button" aria-expanded={!isCollapsed} onClick={() => toggle(section.key)} className="flex w-full items-center justify-between gap-3 px-3 py-2.5 text-left"><span className="min-w-0"><span className="block truncate text-xs font-black">{section.title}</span><span className="mt-0.5 block truncate text-[10px] text-white/40">{section.source} · {rows.length ? `${integer.format(total)} registros` : "sem dados"}</span></span><ChevronDown className={`h-4 w-4 shrink-0 text-white/40 transition-transform ${isCollapsed ? "-rotate-90" : ""}`} /></button>{!isCollapsed && <div className="border-t border-white/10 px-3 pb-3 pt-2">{rows.length ? <div className="space-y-1.5">{rows.slice(0, 8).map((row) => <div key={row.key} className="flex items-center justify-between gap-3 text-xs"><span className="min-w-0 truncate text-white/65" title={row.key}>{row.key}</span><b className="shrink-0 tabular-nums">{integer.format(row.leads)} <small className="font-normal text-white/40">({total ? (row.leads / total * 100).toFixed(1) : "0.0"}%)</small></b></div>)}</div> : <p className="text-[11px] text-white/40">Sem detalhamento sincronizado neste período.</p>}{rows.length > 8 && <p className="mt-2 text-[10px] text-white/35">Exibindo os 8 maiores segmentos.</p>}</div>}</article>; })}</div><p className="mt-3 text-[10px] text-white/40">RD: {integer.format(audience.locatedRegistrations)} de {integer.format(audience.registrations)} cadastros têm cidade ou estado informado ({audience.registrations ? (audience.locatedRegistrations / audience.registrations * 100).toFixed(1) : "0.0"}%).</p></section>;
}
