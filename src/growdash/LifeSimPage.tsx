...
export default function LifeSimPage() {
  ...
  useEffect(() => {
    ...
    const render = () => { frame = requestAnimationFrame(render); if (!visible) return; const elapsed = clock.getElapsedTime(); grid.visible = modeRef.current !== "live"; floorEdges.material.opacity = modeRef.current === "live" ? .3 : .7; signal.intensity = 10 + Math.sin(elapsed * 1.2) * 2; npcObjects.forEach((npc) => { npc.leftArm.rotation.x = -.72 + Math.sin(elapsed * 5 + npc.phase) * .14; npc.rightArm.rotation.x = -.72 - Math.sin(elapsed * 5 + npc.phase) * .14; npc.root.position.y = Math.sin(elapsed * 1.6 + npc.phase) * .012; }); target.lerp(focus, .12); updateCamera(); renderer.render(scene, camera); }; render();
    return () => { cancelAnimationFrame(frame); observer.disconnect(); host.removeEventListener("growdash-life-walls", blueprintHandler); host.removeEventListener("growdash-life-furniture", furnitureHandler); document.removeEventListener("visibilitychange", visibility); renderer.domElement.removeEventListener("pointerdown", down); renderer.domElement.removeEventListener("pointermove", move); renderer.domElement.removeEventListener("pointerup", up); renderer.domElement.removeEventListener("wheel", wheel); renderer.domElement.removeEventListener("keydown", keyDown); scene.traverse((object) => { if (object instanceof THREE.Mesh) { object.geometry.dispose(); const material = object.material; if (Array.isArray(material)) material.forEach((item) => item.dispose()); else material.dispose(); } }); renderer.dispose(); renderer.domElement.remove(); };
  }, []);
