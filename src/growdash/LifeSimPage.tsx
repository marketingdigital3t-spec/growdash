...
export default function LifeSimPage() {
  ...
  const [furniture, setFurniture] = useState<FurnitureBlueprint[]>(DEFAULT_FURNITURE);
  const [furnitureKind, setFurnitureKind] = useState<FurnitureKind>("desk");
  const [npcStatuses, setNpcStatuses] = useState<Record<string, NpcStatus>>({ traffic: "working", finance: "working", commercial: "working", seo: "working", funnels: "working" });
  ...
  useEffect(() => {
    ...
    const npcObjects: Array<{ id: string; root: THREE.Group; base: THREE.Vector3; leftArm: THREE.Group; rightArm: THREE.Group; phase: number }> = [];
    const npcSeeds = [
    ...
    const createNpc = (seed: typeof npcSeeds[number], index: number) => { const root = new THREE.Group(); root.position.set(seed.x, 0, seed.z); root.rotation.y = Math.PI; const body = new THREE.Group(); body.position.y = .74; root.add(body); const skin = new THREE.MeshStandardMaterial({ color: "#d39a74", roughness: .75 }); const outfit = new THREE.MeshStandardMaterial({ color: seed.color, roughness: .52, metalness: .22 }); const torso = new THREE.Mesh(new THREE.CylinderGeometry(.34, .42, .78, 14), outfit); body.add(torso); const head = new THREE.Mesh(new THREE.SphereGeometry(.31, 18, 14), skin); head.position.y = .66; head.scale.set(.9, 1.08, .9); body.add(head); const hair = new THREE.Mesh(new THREE.SphereGeometry(.33, 16, 12, 0, Math.PI * 2, 0, Math.PI * .56), new THREE.MeshStandardMaterial({ color: "#1c1510", roughness: .86 })); hair.position.set(0, .78, .02); hair.scale.set(1.05, .75, 1); body.add(hair); const arm = (side: number) => { const pivot = new THREE.Group(); pivot.position.set(side * .4, .2, 0); const limb = new THREE.Mesh(new THREE.CapsuleGeometry(.085, .48, 4, 10), outfit); limb.rotation.z = side * -.42; limb.position.y = -.28; pivot.add(limb); body.add(pivot); return pivot; }; const leftArm = arm(-1); const rightArm = arm(1); for (const x of [-.15, .15]) { const leg = new THREE.Mesh(new THREE.BoxGeometry(.15, .38, .18), new THREE.MeshStandardMaterial({ color: "#17130e", roughness: .75 })); leg.position.set(x, -.52, .18); leg.rotation.x = Math.PI / 2; body.add(leg); } const halo = new THREE.Mesh(new THREE.RingGeometry(.38, .49, 24), new THREE.MeshBasicMaterial({ color: seed.color, transparent: true, opacity: .55, side: THREE.DoubleSide })); halo.rotation.x = -Math.PI / 2; halo.position.y = .02; root.add(halo); root.userData.label = seed.name; scene.add(root); npcObjects.push({ id: seed.id, root, base: root.position.clone(), leftArm, rightArm, phase: index * .72 }); };
    ...
    const render = () => { frame = requestAnimationFrame(render); if (!visible) return; const elapsed = clock.getElapsedTime(); grid.visible = modeRef.current !== "live"; floorEdges.material.opacity = modeRef.current === "live" ? .3 : .7; signal.intensity = 10 + Math.sin(elapsed * 1.2) * 2; npcObjects.forEach((npc, index) => { const status = currentNpcStatuses[npc.id] || "working"; if (status === "walking") { npc.root.position.lerp(new THREE.Vector3(npc.base.x + Math.sin(elapsed + npc.phase) * 1.5, .02, npc.base.z + Math.cos(elapsed * .8 + npc.phase) * 1.1), .12); npc.root.rotation.y = Math.atan2(Math.cos(elapsed + npc.phase), -Math.sin(elapsed * .8 + npc.phase)); npc.leftArm.rotation.x = Math.sin(elapsed * 3 + npc.phase) * .58; npc.rightArm.rotation.x = -Math.sin(elapsed * 3 + npc.phase) * .58; } else if (status === "free") { const lounge = new THREE.Vector3(-7 + index * .25, 0, 6.5 + index * .12); npc.root.position.lerp(lounge, .04); npc.root.rotation.y = .35; npc.leftArm.rotation.x = .12; npc.rightArm.rotation.x = -.12; } else { npc.root.position.lerp(npc.base, .11); npc.root.position.y = Math.sin(elapsed * 1.6 + npc.phase) * .012; npc.root.rotation.y = Math.PI; npc.leftArm.rotation.x = -.72 + Math.sin(elapsed * 5 + npc.phase) * .14; npc.rightArm.rotation.x = -.72 - Math.sin(elapsed * 5 + npc.phase) * .14; } }); target.lerp(focus, .12); updateCamera(); renderer.render(scene, camera); }; render();
    return () => { cancelAnimationFrame(frame); observer.disconnect(); host.removeEventListener("growdash-life-walls", blueprintHandler); host.removeEventListener("growdash-life-furniture", furnitureHandler); host.removeEventListener("growdash-life-npcs", npcStatusHandler); document.removeEventListener("visibilitychange", visibility); renderer.domElement.removeEventListener("pointerdown", down); renderer.domElement.removeEventListener("pointermove", move); renderer.domElement.removeEventListener("pointerup", up); renderer.domElement.removeEventListener("wheel", wheel); renderer.domElement.removeEventListener("keydown", keyDown); scene.traverse((object) => { if (object instanceof THREE.Mesh) { object.geometry.dispose(); const material = object.material; if (Array.isArray(material)) material.forEach((item) => item.dispose()); else material.dispose(); } }); renderer.dispose(); renderer.domElement.remove(); };
  }, []);
