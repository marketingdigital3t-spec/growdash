...
function ActionButton({ children, primary = false, neutral = false, onClick }: { children: ReactNode; primary?: boolean; neutral?: boolean; onClick?: () => void }) {
  return <button type="button" onClick={onClick} className={neutral ? "silver-action" : primary ? "gold-action" : "gd-button"}>{children}</button>;
}
...
function TicketsModule() {
  ...
  return (
    <Page title="Chamados" description="Abra, acompanhe e mantenha as solicitações do seu workspace organizadas." action={<ActionButton neutral onClick={() => { setEditingTicket(null); setForm({ title: "", description: "", requester_name: user?.user_metadata?.full_name || user?.email || "", requested_at: new Date().toISOString().slice(0, 10), category: "general", priority: "normal" }); setOpen(true); }}><Plus className="h-4 w-4" /> Novo chamado</ActionButton>}>
