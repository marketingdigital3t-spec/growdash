...
export function NeuralCommandCore3D({ expanded, entering = false, onEnter, health = "no-data", healthScore = null }: NeuralCommandCore3DProps) {
  ...
  useEffect(() => {
    ...
    // The production brain is a real GLB mesh (not a flat image or stacked
    // DOM/SVG layers). The procedural cortex remains as a resilient fallback
    // for offline, cache-miss or GPU-constrained sessions.
    const fallbackCortex = new THREE.Group();
    fallbackCortex.add(cortex, folds, particles);
    core.add(fallbackCortex);
    let glbBrain: THREE.Group | null = null;
    let glbLoaded = false;
    let disposed = false;
    const glbMaterial = new THREE.MeshPhysicalMaterial({
      color: "#d8dce1",
      emissive: "#11151a",
    ...
    new GLTFLoader().load("/models/growdash-brain.glb", (gltf) => {
      if (disposed) {
      ...
      model.scale.setScalar(4.15 / largestDimension);
      model.rotation.set(-.08, Math.PI * .12, 0);
      model.traverse((object) => {
        ...
        if (Array.isArray(original)) original.forEach((entry) => entry.dispose()); else original?.dispose();
        mesh.material = glbMaterial;
      });
      fallbackCortex.visible = false;
      core.add(model);
      glbBrain = model;
      glbLoaded = true;
    }, undefined, () => {
      // The fallback is intentionally already on screen; a model request can
      // fail without blanking the intelligence module.
      fallbackCortex.visible = true;
    });
    ...
    const applyPalette = () => {
      ...
      glbMaterial.color.set(health === "critical" ? nextHealthColor : neuralLight);
      glbMaterial.emissive.set(health === "critical" ? "#2a0707" : "#11151a");
