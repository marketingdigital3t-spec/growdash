...
export function NeuralCommandCore3D({ expanded, entering = false, onEnter, health = "no-data", healthScore = null }: NeuralCommandCore3DProps) {
  ...
  useEffect(() => {
    ...
    const wireMaterial = new THREE.MeshBasicMaterial({ color: neuralLight, transparent: true, opacity: .23, wireframe: true });
    ...
    const linesMaterial = new THREE.LineBasicMaterial({ color: neuralLight, transparent: true, opacity: .42 });
    ...
    for (let index = 0; index < 38; index++) {
      ...
      const y = -1.24 + (index % 12) * .22;
      const z = -.72 + Math.floor(index / 12) * .5;
      const points = Array.from({ length: 7 }, (_, pointIndex) => {
        const t = pointIndex / 6;
        return new THREE.Vector3(side * (.34 + Math.sin(t * Math.PI) * (1.08 + (index % 3) * .08)), y + Math.sin(t * Math.PI * 2 + index) * .12, z + Math.cos(t * Math.PI * 2 + index * .7) * .16);
      });
