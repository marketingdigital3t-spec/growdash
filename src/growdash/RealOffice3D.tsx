...
export function RealOffice3D({ agents, onSelectAgent, onStatusChange }: Props) {
  ...
  useEffect(() => {
    ...
    const createNpc = (agent: OfficeNpc, index: number) => {
      ...
      // Real GLB avatars live on this independent anchor. Keeping it separate
      // from the procedural limbs lets the loaded model use the same movement,
      // selection and cutaway behaviour without depending on a third-party rig.
      const avatarAnchor = new THREE.Group(); avatarAnchor.name = `${agent.id}-avatar`; root.add(avatarAnchor);
      ...
      scene.add(root); npcObjects.set(agent.id, { root, body, avatarAnchor, leftArm, rightArm, legs, base: root.position.clone() });
    };
    agentsRef.current.forEach(createNpc);

    // Aurora is an original, compact GLB asset generated for Growdash. One
    // detailed character keeps the office responsive on mobile while the
    // remaining procedural agents retain their live work animations.
    let disposed = false;
    const avatarAgent = agentsRef.current[0];
    if (avatarAgent) new GLTFLoader().load("/models/growdash-npc-aurora.glb", (gltf) => {
      if (disposed) return;
      const npc = npcObjects.get(avatarAgent.id);
      if (!npc) return;
      const avatar = gltf.scene;
      const bounds = new THREE.Box3().setFromObject(avatar);
      const size = bounds.getSize(new THREE.Vector3());
      const center = bounds.getCenter(new THREE.Vector3());
      const scale = 2.12 / Math.max(size.y, .01);
      avatar.position.set(-center.x * scale, -bounds.min.y * scale + .03, -center.z * scale);
      ...
      avatar.traverse((object) => {
        ...
        mesh.receiveShadow = true;
        mesh.userData.npcId = avatarAgent.id;
        pickables.push(mesh);
      });
      npc.avatarAnchor.add(avatar);
      // Do not layer a geometric mannequin beneath the actual GLB model.
    ...
    const animate = () => {
      ...
      for (const [index, agent] of agentsRef.current.entries()) {
        ...
        // The GLB needs a gentle life-like idle too, even when its source has
        // no skeleton animation. It remains a volumetric model at every angle.
        npc.avatarAnchor.position.y = Math.sin(phase * 1.7) * .012;
        npc.avatarAnchor.rotation.z = Math.sin(phase * 1.05) * .012;
      }
