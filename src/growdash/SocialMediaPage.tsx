        ...
        <>
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            ...
            <TabsContent value="content" className="space-y-5">
              <section className="gd-panel overflow-hidden p-0">
                <div className="h-20 bg-[radial-gradient(ellipse_at_top_right,hsl(var(--primary)/.24),transparent_58%),linear-gradient(115deg,hsl(var(--card)),hsl(var(--muted)/.35))]" />
                <div className="px-5 pb-5 sm:px-7">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div className="flex min-w-0 items-end gap-4">
                      <div className="-mt-10 grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-full border-4 border-card bg-primary/10 text-primary shadow-lg">
                        {selected?.profile_picture_url ? <img src={selected.profile_picture_url} alt="" className="h-full w-full object-cover" /> : <Instagram className="h-8 w-8" />}
                      </div>
                      <div className="min-w-0 pb-1"><h3 className="truncate text-lg font-black">{selected?.display_name || "Perfil Instagram"}</h3><p className="truncate text-sm text-muted-foreground">@{selected?.username || "perfil"}</p></div>
                    </div>
                    <div className="flex gap-5 text-sm"><span><b className="block font-black">{number.format(selected?.media_count ?? media.length)}</b><small className="text-muted-foreground">publicações</small></span><span><b className="block font-black">{number.format(selected?.followers_count ?? 0)}</b><small className="text-muted-foreground">seguidores</small></span><span><b className="block font-black">{number.format(totals.interactions)}</b><small className="text-muted-foreground">interações</small></span></div>
                  </div>
                </div>
              </section>
              ...
              <div className="grid grid-cols-2 gap-1 sm:grid-cols-3 lg:grid-cols-4">
                ...
                {!media.length && <div className="col-span-full"><Empty text="Nenhum conteúdo publicado no período selecionado." /></div>}
              </div>
            </TabsContent>
