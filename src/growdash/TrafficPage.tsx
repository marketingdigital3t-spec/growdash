...
import { FunnelAudienceProfile, useAudienceProfileData } from "@/components/funnel-analysis/FunnelAudienceProfile";
...
export default function TrafficPage() {
  ...
  const selectedForDisplay = adAccountIds.length ? adAccountIds : visibleAccounts.map((item) => item.id);
  ...
  return (
    <div className={cn(
      ...
      {activeTab !== "campaigns" && activeTab !== "ai" && activeTab !== "presentation" && <section className="flex flex-col gap-2 rounded-xl border border-border bg-card p-3 sm:flex-row sm:items-center">
        ...
        <AccountMultiSelect accounts={visibleAccounts.map((item) => ({ id: item.id, name: item.name }))} selectedIds={adAccountIds} onChange={setAdAccountIds} className="h-9 sm:max-w-md" />
...
function BudgetAccountCard({ item }: { item: BudgetAnalysisItem }) {
  ...
  const severitySurface = item.severity === "critical"
    ? "border-red-500/60 bg-red-500/[.12]"
    : item.severity === "warning"
      ? "border-amber-500/55 bg-amber-500/[.1]"
      : "border-emerald-500/55 bg-emerald-500/[.1]";
...
function AIAndLeadReports({ accountId, accountIds, accountName, accounts, onAccountChange }: { accountId: string; accountIds: string[]; accountName?: string; accounts: Array<{ id: string; name: string }>; onAccountChange: (id: string) => void }) {
  ...
  const audience = useAudienceProfileData({ deals, campaignIds: audienceCampaignIds, startDate, endDate });
  const { toast } = useToast();
