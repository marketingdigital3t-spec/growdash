  ...
  return (
    <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div>
        ...
        <h1 className="text-2xl font-black tracking-tight text-[#171512] sm:text-3xl">{title}</h1>
        <p className="mt-1 max-w-2xl text-sm text-[#77716a]">{description}</p>
      </div>
      {actions ?? (
...
export function MetricCard({ label, value, change, emphasis }: { label: string; value: string; change: string; emphasis?: boolean }) {
  ...
  return (
    <div className={cn("gd-panel p-4", emphasis && "border-[#e4bc4b]/60 bg-gradient-to-br from-[#fffdf7] to-[#faf3dd]")}>
      <div className="flex items-center justify-between gap-2">
        <span className="text-[11px] font-semibold text-[#77716a]">{label}</span>
      ...
      <div className="mt-3 text-[22px] font-black tracking-tight text-[#211e1a]">{value}</div>
    </div>
  );
}
