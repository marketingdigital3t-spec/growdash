...
export function normalizeRDQueryScope(scope: RDQueryScope): RDQueryScope {
  return { ...scope, accountIds: Array.from(new Set(scope.accountIds.filter(Boolean))).sort(), funnelIds: Array.from(new Set(scope.funnelIds.filter(Boolean))).sort() };
}

export function rdScopeQueryKey(scope: RDQueryScope) {
  const normalized = normalizeRDQueryScope(scope);
  return [normalized.accountIds.join(","), normalized.funnelIds.join(","), normalized.startDate.toISOString(), normalized.endDate.toISOString(), normalized.dateRule] as const;
}
...
export function isDealInRDQueryScope(deal: ScopedDeal, scope: RDQueryScope) {
  const normalized = normalizeRDQueryScope(scope);
  return (!normalized.accountIds.length || normalized.accountIds.includes(deal.ad_account_id || "")) && (!normalized.funnelIds.length || normalized.funnelIds.includes(deal.rd_funnel_id || ""));
}
