...
export default function Campaigns() {
  ...
  useEffect(() => {
    ...
    const totals = campaignTotalsScrollRef.current;
    if (!table || !totals || activeTab !== "campaigns") return;

    const sync = () => { totals.scrollLeft = table.scrollLeft; };
    const matchViewportWidth = () => {
      // The campaign list has a vertical scrollbar; reserve that same width in
      // the separate totals block so both horizontal viewports finish together.
      totals.style.marginRight = `${Math.max(0, table.offsetWidth - table.clientWidth)}px`;
      sync();
    };
    matchViewportWidth();
    table.addEventListener("scroll", sync, { passive: true });
    const observer = new ResizeObserver(matchViewportWidth);
    observer.observe(table);
    return () => {
      table.removeEventListener("scroll", sync);
      observer.disconnect();
    };
  }, [activeTab, camp.colWidths, filtered.length]);
                  ...
                  <table className="w-full caption-bottom text-sm [&_td]:overflow-hidden [&_td]:whitespace-nowrap" style={{ tableLayout: "fixed", width: "max-content" }}>
                    <TableHeader className="sticky top-0 z-50 shadow-[0_2px_8px_rgba(0,0,0,.08)]">
                      <TableRow className="campaign-metric-header h-10 border-b border-border hover:bg-transparent [&>th]:h-10 [&>th]:px-3 [&>th]:py-1 dark:border-[#28251e]">
                        ...
                        {showColumn("leads") && <ResizableHead colKey="leads" width={camp.colWidths.leads} onResize={camp.startResize("leads")} sortable sortableKey="leads" sortKey={sortKey} sortAsc={sortAsc} onSort={handleSort} align="right" title="Leads atribuídos à campanha pelos eventos da Meta">Leads atribuídos</ResizableHead>}
                        {showColumn("cpl") && <ResizableHead colKey="cpl" width={camp.colWidths.cpl} onResize={camp.startResize("cpl")} sortable sortableKey="cpl" sortKey={sortKey} sortAsc={sortAsc} onSort={handleSort} align="right">Custo por lead</ResizableHead>}
                        ...
                        {showColumn("sales") && <ResizableHead colKey="sales" width={camp.colWidths.sales} onResize={camp.startResize("sales")} sortable sortableKey="salesCount" sortKey={sortKey} sortAsc={sortAsc} onSort={handleSort} align="right" title="Vendas confirmadas atribuídas à campanha pelo ID, UTM ou nome persistido">Vendas atribuídas</ResizableHead>}
                        {showColumn("cpa") && <ResizableHead colKey="cpa" width={camp.colWidths.cpa} onResize={camp.startResize("cpa")} sortable sortableKey="cpa" sortKey={sortKey} sortAsc={sortAsc} onSort={handleSort} align="right">Custo por venda</ResizableHead>}
