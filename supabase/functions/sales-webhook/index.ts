...
function secretsMatch(received: string | null | undefined, expected: string | null | undefined) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // TheMembers oferece apenas o campo "Token de segurança". Aceitamos o
    // token no corpo somente para esse provedor; os demais seguem usando o
    // cabeçalho explícito, evitando que um campo comum de payload vire senha.
    const providerBodyToken = sourceProvider === "themembers"
      ? stringValue(valueAt(payload, ["token", "security_token", "data.token", "data.security_token"]))
      : "";
    const secret = req.headers.get("x-growdash-webhook-secret")?.trim()
      || req.headers.get("x-webhook-token")?.trim()
      || authorization
      || providerBodyToken;
    if (!integration || !secretsMatch(secret, integration.webhook_secret)) return json({ error: "Não autorizado" }, 401);
    ...
    // A mesma transação pode ter compra, reembolso e chargeback. A chave de
    // idempotência precisa distinguir os eventos pelo payload, não bloquear
    // todos os estados futuros apenas porque compartilham o número do pedido.
