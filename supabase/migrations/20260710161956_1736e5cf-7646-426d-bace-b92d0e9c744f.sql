GRANT INSERT, UPDATE ON public.conversation_access_codes TO authenticated;

DROP POLICY IF EXISTS "Users can create own conversation access code" ON public.conversation_access_codes;
CREATE POLICY "Users can create own conversation access code"
...
DROP POLICY IF EXISTS "Users can rotate own conversation access code" ON public.conversation_access_codes;
CREATE POLICY "Users can rotate own conversation access code"
ON public.conversation_access_codes
FOR UPDATE
TO authenticated
USING (
...
REVOKE ALL ON FUNCTION public.generate_chat_access_code() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.generate_chat_access_code() TO service_role;

REVOKE ALL ON FUNCTION public.current_chat_code_day() FROM PUBLIC, anon;
