-- Managed users may author campaign drafts. Keep the draft for audit while
-- allowing the user account to be removed safely.
ALTER TABLE public.campaign_drafts
  ALTER COLUMN created_by DROP NOT NULL;

ALTER TABLE public.campaign_drafts
  DROP CONSTRAINT IF EXISTS campaign_drafts_created_by_fkey;

ALTER TABLE public.campaign_drafts
  ADD CONSTRAINT campaign_drafts_created_by_fkey
  FOREIGN KEY (created_by) REFERENCES auth.users(id) ON DELETE SET NULL;

-- Social media data is separate from paid-media insights. Tokens stay in the
-- protected integrations table; these browser-readable tables contain only
-- profile, media and aggregated metrics.
